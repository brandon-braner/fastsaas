...
def auto_mount_routes():
    ...
    subfolders = [f for f in os.listdir(current_folder) if (os.path.isdir(os.path.join(current_folder, f)) and not f.startswith("__"))]
    ...
    for folder in subfolders:
        ...
        for route in routes:
            router = importlib.import_module(f"{name}.{route}_routes")
            if route == "api":
